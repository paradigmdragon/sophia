# Epidora2.md (소피아 실행용 오류검증 SSOT) v1.0
...
2. 판정 임계값: `score >= 2.0`일 때만 `CONFIRMED`
3. 의심 허용: `1.0~1.5`는 `SUSPECTED` + 추가 질문(EPI)만 수행
4. 다중 오류 허용: 주 1개 + 부 0~2개
5. 자동 반영 금지: 소피아는 “검토 요청 문서”까지만, 반영은 사용자 결정

---

## 2) Quick Triage (초기 라우팅 3문장)
아래 3개 질문으로 “우선 후보”를 잡는다.

1) 위계/정의/포함/자기참조가 무너졌는가? → 오류3 우선  
2) 생성 국면(규칙/의도/정의)과 적용 국면(판정/실행/관측)이 섞였는가? → 오류2 우선  
3) 개입(선택/해석/편집)이 숨겨져 객관처럼 서술되는가? → 오류6 우선  

> triage는 “확정”이 아니라 “후보 선정”이다. 최종은 스코어로 결정.
...
**교정(Rewrite)**
- 분리: “단어(이름)”와 “대상(현상/사례)” 분리
- 재정의: 대체 정의 2개 생성 후 각각으로 재판정
- 재검증: 대체 정의 변경에도 결론이 동일하면 오류5 가능성 점검

**Antidote (EPI 매핑)**
- `EPI_ID: EPI-01`
- `canonical_question: (여기에 Epidora 질문 원문 1줄)`
...
**교정(Rewrite)**
- 분리: 생성 문장 1개 / 적용 문장 1개로 분해
- 태깅: 각 문장에 `PHASE=GENERATE` 또는 `PHASE=APPLY` 라벨
- 재검증: 분리 후에도 모순이 남으면 오류3/6 후보 재평가
...
**조건 체크**
- E3-C1: 포함 관계 역전/자기포함 발생
- E3-C2: 정의/판정의 자기참조 발생
- E3-C3: 보조 신호로 무한 후퇴 징후 존재

**교정(Rewrite)**
- 분리: 상위(규칙/정의) 블록과 하위(사례/대상) 블록을 문서/섹션으로 분리
- 범위: 상위 규칙에 적용 범위/예외/자기참조 금지 조건 명시
- 재검증: 분리 후 “규칙→사례” 단방향 적용만 허용
...
**교정(Rewrite)**
- 분리: 결과/과정/전환점(전이 조건) 3요소로 재서술
- 재구성: 최소 3단계 흐름(전/중/후)로 복원
- 재검증: 전환 조건이 명시되면 오류2/6 동반 여부 점검
...
**교정(Rewrite)**
- 분리: 관측(사실) / 해석(틀) / 결론 3단 분리
- 비교: 대체 프레임 1개 적용해 동일 데이터 재판정
- 재검증: 프레임 변경에도 동일 결론이 남는 핵심 불변량 도출
...
**추가 강화 조건(권장)**
- E6-C4: 개입이 결과/행동/의사결정에 영향을 주는 피드백 루프가 존재(관찰이 결과값을 바꿈)
...
**교정(Rewrite)**
- 분리: 원문/로그(사실) vs 요약/평가(해석) vs 목적(의도) 분리
- 표시: “해석 라벨” + 근거(원본 포인터) 필수 부착
- 재검증: 원본 없이 단정하는 문장 금지(근거 링크 없으면 자동 `SUSPECTED`)
...
- 저장: 원본(불변) + 리포트(가변) 분리 저장, 리포트에는 원본 포인터 필수(오류6 방지)

---

## 8) 미결 슬롯(필수 입력)
이 파일만 SSOT로 쓰려면 아래 6개 문장만 채우면 완성된다.
- `EPI-01 canonical_question`
- `EPI-02 canonical_question`
- `EPI-03 canonical_question`
- `EPI-04 canonical_question`
- `EPI-05 canonical_question`
- `EPI-06 canonical_question`

> 원문 문구를 넣으면, 소피아는 “판정 → 즉시 질문”까지 Epidora2.md 하나로 수행한다.